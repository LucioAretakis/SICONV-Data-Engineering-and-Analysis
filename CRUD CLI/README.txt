...
Bash
python crud_cli.py
Siga as instruções numeradas no menu.

🛡️ Tratamento de Erros
O sistema está preparado para lidar com:

Falhas de Conexão: Encerra o script com uma mensagem clara caso o banco de dados esteja offline.

Transações Interrompidas: Em caso de erro durante a inserção ou exclusão, o sistema realiza um rollback automático, não afetando a consistência do banco.

Validação de Entrada: Tratamento básico para garantir que tipos de dados (como números de convênio) sejam processados corretamente.